* build 1971 towns dta with from deo entry
*-------------------------------------------------------------------------------
* prelim
...
* use 1971 sheet
  import delimited using "$input/town/deo/1971.csv", clear varnames(2) stringcols(_all) favorstrfixed
*-------------------------------------------------------------------------------
...
* rename numeric data columns
  ...
  rename (wrk_t_t wrk_t_m wkr_t_f) (workers_t workers_m workers_f)
  rename (wrk_1_t wrk_1_m wrk_1_f) (wrk_1t wrk_1m wrk_1f)
  rename (wrk_2_t wrk_2_m wrk_2_f) (wrk_2t wrk_2m wrk_2f)
  rename (wrk_3_t wrk_3_m wrk_3_f) (wrk_3t wrk_3m wrk_3f)
  rename (wrk_4_t wrk_4_m wrk_4_f) (wrk_4t wrk_4m wrk_4f)
  rename (wrk_5a_t wrk_5a_m wrk_5a_f) (wrk_5at wrk_5am wrk_5af)
  rename (wrk_5b_t wrk_5b_m wrk_5b_f) (wrk_5bt wrk_5bm wrk_5bf)
  rename (wrk_6_t wrk_6_m wrk_6_f) (wrk_6t wrk_6m wrk_6f)
  rename (wrk_7_t wrk_7_m wrk_7_f) (wrk_7t wrk_7m wrk_7f)
  rename (wrk_8_t wrk_8_m wrk_8_f) (wrk_8t wrk_8m wrk_8f)
  rename (wrk_9_t wrk_9_m wrk_9_f) (wrk_9t wrk_9m wrk_9f)
...
* clean area - generate numeric var and convert to sq kilometres
  ...
  * standardise to sq kilometres
  ...
  replace area_sqkm = area_num*2.58999 if regexm(area_units, "mile")
...
* transfer tehsil/taluk info from block and subdivision to subdistrict
  replace subdistrict = block if regexm(block, "(ta?e?t?h?a?sil|taluka?)")
...
* subdistrict classification
  ...
  * independent sub taluk
    replace subdist_type = "ind. sub-taluk" if regexm(subdistrict, " i\.s\.t")
    replace subdistrict = regexr(subdistrict, " i\.s\.t","")
...
* lowland/highland (l/h) - not relevant
  qui replace civic_code = "()" if regexm(townsieve,"( l| h)$") & civic_code == ""
  qui replace townsieve = regexr(townsieve,"( l| h)$","")
  qui replace civic_status = "lowland/highland" if civic_code == "()"

* township (ts)
  qui replace civic_code = "(ts)" if regexm(townsieve,"( ts)$|township") & civic_code == ""
  qui replace townsieve = regexr(townsieve,"( ts)$|township","")
  qui replace civic_status = "township" if civic_code == "(ts)"

* town area (ta)
  qui replace civic_code = "(ta)" if regexm(townsieve, "( ta)$") & civic_code == ""
...
* notified area committee (nac)
  qui replace civic_code = "(nac)" if regexm(townsieve, "( na?c?| notified)$") & civic_code == ""
  qui replace townsieve = regexr(townsieve, "( na?c?| notified)$","")
...
* cantonment (cantt)
  qui replace civic_code = "(cantt)" if regexm(townsieve," cantt| chh?ava?ni|cantonment") & civic_code == ""
  qui replace townsieve = regexr(townsieve,"( cantt?| chh?ava?ni)$|cantonment","")
...
* urban outgrowth (og)
  ...
  qui replace civic_status = "urban outgrowh" if civic_code == "(og)"
...
* census towns
  qui replace civic_code = "(ct)" if regexm(townsieve,"( ct)$") & civic_code == ""
  qui replace townsieve = regexr(townsieve,"( ct)$","")
  qui replace civic_status = "census town" if civic_code == "(ct)"

* unclassified census towns ()
	qui replace civic_code = "()" if civic_code == ""
	qui replace civic_status = "unknown/not available" if civic_code == "()" & civic_status != "lowland/highland"
...
* assign codes (india_bridge ado programs)
  indiabridge, y(1971) s(sname) d(dname)
...
*-------------------------------------------------------------------------------
  * generate matchit string
    ...
    * oncatenate name and civic status
    ...
    replace string = subinstr(string,")","",.)
    replace string = ustrtrim(string)
...
* absorb outgrowths and remove double counting of (ua)
  ...
  * update civic status
  bysort ua sortvar: gen ua_civ = subinstr(civic_code,")"," + og)",.) if has_og == 1 & has_ua == 0
  replace ua_civ = "(ua)" if has_ua == 1
...
* order, keep and sort
  order isostate scode sname dcode dname sdivname sdname sdclass bname ua tgroup tsubgroup tname civic_code
  sort scode dcode sdivname sdname bname ua tgroup tsubgroup tname

* add 1971 suffix to varname
  rename * *_1971

* sanity check
  duplicates tag dcode_1971 tname_1971 civic_code_1971, gen(tag)

  * find duplicated observations, collapse based on non-missing data
    local bylist "scode_1971 dcode_1971 tname_1971 civic_code_1971"
    ds
  ...
  * no duplicates detected. n = 3520j

tempfile 71entry
save `71entry', replace

*-------------------------------------------------------------------------------
* use india_bridge for 1971 identification
  use $india_bridge/output/india_bridge, clear

* keep data for identification at 1961-1971 district level
  keep *_1961 *_1971
  * make 1971 identification unique
  duplicates drop dcode_1971, force
  isid dcode_1971
  * n = 357

* merge with 1971townlist to track 1971 district code
  merge 1:m dcode_1971 using `71entry'
/*
...
* label variables
  ...
  * metadata
  label var filename_1971 "1971 pdf file name"
  label var pdf_page_1971 "1971 pdf page number"

  * id vars
  label var sdivname_1971 "1971 subdivision name"
  label var sdname_1971 "1971 subdistrict name"
  label var sdclass_1971 "1971 subdistrict classification"
  label var bname_1971 "1971 block name"
  label var tname_1971 "1971 town name"
  label var civic_code_1971 "1971 civic status of town"
  label var civic_status_1971 "1971 civic code interpretation for town"
  label var string "1971 matchit string, = dcode61 + tname71 + civic71"
	label var tgroup_1971 "1971 urban agglomeration (definition changed from town group)"
	label var tsubgroup_1971 "1971 agglomeration sub-group"

  * area & houses
    label var area_sqkm_1971 "1971 area in sq kilometres"
    label var nhouses_1971 "1971 number of houses"
    label var nhouseholds_1971 "1971 number of households"

  * town population
    label var townpop_1971 "1971 town population"
    label var townpop_m_1971 "1971 population (male)"
    label var townpop_f_1971 "1971 population (female)"
  * sc
    label var sc_t_1971 "1971 number of scheduled castes (total)"
    label var sc_m_1971 "1971 number of scheduled castes (male)"
    label var sc_f_1971 "1971 number of scheduled castes (female)"
  * st
    label var st_t_1971 "1971 number of scheduled tribes (total)"
    label var st_m_1971 "1971 number of scheduled tribes (male)"
    label var st_f_1971 "1971 number of scheduled tribes (female)"
  * literates
    label var edu_t_1971 "1971 number of literates (total)"
    label var edu_m_1971 "1971 number of literates (male)"
    label var edu_f_1971 "1971 town number of literates (female)"
  * workers
    label var workers_t_1971 "1971 number of workers (total)"
    label var workers_m_1971 "1971 number of workers (male)"
    label var workers_f_1971 "1971 number of workers (female)"
  * 1 workers
    label var wrk_1t_1971 "1971 number of category 1 workers - cultivators (total)"
    label var wrk_1m_1971 "1971 number of category 1 workers - cultivators (male)"
    label var wrk_1f_1971 "1971 number of category 1 workers - cultivators (female)"
  * 2 workers
    label var wrk_2t_1971 "1971 number of category 2 workers - agricultural labourers (total)"
    label var wrk_2m_1971 "1971 number of category 2 workers - agricultural labourers (male)"
    label var wrk_2f_1971 "1971 number of category 2 workers - agricultural labourers (female)"
  * 3 workers
    label var wrk_3t_1971 "1971 number of category 3 workers - livestock fishing etc (total)"
    label var wrk_3m_1971 "1971 number of category 3 workers - livestock fishing etc (male)"
    label var wrk_3f_1971 "1971 number of category 3 workers - livestock fishing etc (female)"
  * 4 workers
    label var wrk_4t_1971 "1971 number of category 4 workers - mining and quarrying (total)"
    label var wrk_4m_1971 "1971 number of category 4 workers - mining and quarrying (male)"
    label var wrk_4f_1971 "1971 number of category 4 workers - mining and quarrying (female)"
  * 5a workers
    label var wrk_5at_1971 "1971 number of category 5a workers - manufacturing, household industry (total)"
    label var wrk_5am_1971 "1971 number of category 5a workers - manufacturing, household industry (male)"
    label var wrk_5af_1971 "1971 number of category 5a workers - manufacturing, household industry (female)"
  * 5b workers
    label var wrk_5bt_1971 "1971 number of category 5b workers - manufacturing, household industry (total)"
    label var wrk_5bm_1971 "1971 number of category 5b workers - manufacturing, household industry (male)"
    label var wrk_5bf_1971 "1971 number of category 5b workers - manufacturing, household industry (female)"
  * 6 workers
    label var wrk_6t_1971 "1971 number of category 6 workers - construction (total)"
    label var wrk_6m_1971 "1971 number of category 6 workers - construction (male)"
    label var wrk_6f_1971 "1971 number of category 6 workers - construction (female)"
  * 7 workers
    label var wrk_7t_1971 "1971 number of category 7 workers - trade and commerce (total)"
    label var wrk_7m_1971 "1971 number of category 7 workers - trade and commerce (male)"
    label var wrk_7f_1971 "1971 number of category 7 workers - trade and commerce (female)"
  * 8 workers
    label var wrk_8t_1971 "1971 number of category 8 workers - transport and storage (total)"
    label var wrk_8m_1971 "1971 number of category 8 workers - transport and storage (male)"
    label var wrk_8f_1971 "1971 number of category 8 workers - transport and storage (female)"
  * 9 workers
    label var wrk_9t_1971 "1971 number of category 9 workers - other services (total)"
    label var wrk_9m_1971 "1971 number of category 9 workers - other services (male)"
    label var wrk_9f_1971 "1971 number of category 9 workers - other services (female)"
  * non workers
    label var wrk_nont_1971 "1971 number of non workers (total)"
    label var wrk_nonm_1971 "1971 number of non workers (male)"
    label var wrk_nonf_1971 "1971 number of non workers (female)"
*-------------------------------------------------------------------------------


* store variable lists in local
  local numvars "area_sqkm_1971 nhouses_1971 nhouseholds_1971 townpop_1971 townpop_m_1971 townpop_f_1971 sc_t_1971 sc_m_1971 sc_f_1971 st_t_1971 st_m_1971 st_f_1971 edu_t_1971 edu_m_1971 edu_f_1971 workers_t_1971 workers_m_1971 workers_f_1971 wrk_1t_1971 wrk_1m_1971 wrk_1f_1971 wrk_2t_1971 wrk_2m_1971 wrk_2f_1971 wrk_3t_1971 wrk_3m_1971 wrk_3f_1971 wrk_4t_1971 wrk_4m_1971 wrk_4f_1971 wrk_5at_1971 wrk_5am_1971 wrk_5af_1971 wrk_5bt_1971 wrk_5bm_1971 wrk_5bf_1971 wrk_6t_1971 wrk_6m_1971 wrk_6f_1971 wrk_7t_1971 wrk_7m_1971 wrk_7f_1971 wrk_8t_1971 wrk_8m_1971 wrk_8f_1971 wrk_9t_1971 wrk_9m_1971 wrk_9f_1971 wrk_nont_1971 wrk_nonm_1971 wrk_nonf_1971"
  local idvars "string isostate_1971 scode_1971 sname_1971 dcode_1971 dname_1971 sdivname_1971 sdname_1971 sdclass_1971 bname_1971 ua_1971 tgroup_1971 tsubgroup_1971 tname_1971 civic_code_1971"
  local meta "filename_1971 pdf_page_1971"
  local 61vars "sname_1961 dname_1961 dcode_1961"

* keep, order and sort
  keep `meta' `numvars' `idvars' `61vars'
  sort scode_1971 dcode_1971 sdivname_1971 sdname_1971 bname_1971 tgroup_1971 tsubgroup_1971 tname_1971 civic_code_1971

* generate id
  gen id_1971 = string(_n, "%05.0f")
  label var id_1971 "1971 town id"
  order id_1971 `meta' `idvars' `numvars' `61vars'

* compress and save
  compress
  // isid string
  isid id_1971
  save $dchb/input/town/build/1971, replace
*==============================================================================
